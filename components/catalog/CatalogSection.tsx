      ...
      <div className="space-y-10">
        <div className="overflow-hidden rounded-md border border-line bg-white shadow-[0_28px_70px_rgba(17,24,20,0.08)]">
          <div className="grid md:grid-cols-[0.42fr_0.58fr]">
            <div className="flex min-h-[13rem] flex-col justify-between bg-[linear-gradient(135deg,#6a9955_0%,#4d7f41_100%)] p-6 text-white sm:p-8">
              <div className="space-y-4">
                <p className="text-[0.7rem] font-semibold uppercase tracking-[0.24em] text-white/70">
                  {dictionary.catalog.hero.eyebrow}
                </p>
                <h2 className="font-serif text-5xl leading-none tracking-[-0.05em] text-white sm:text-6xl">
                  {category.name}
                </h2>
            ...
            <div className="relative min-h-[13rem] border-t border-line bg-[#f0efe9] md:border-t-0 md:border-l">
              <Image
                alt=""
                aria-hidden="true"
        ...
        <div className="space-y-10">
          {category.groups.map((group) => (
            <section className="space-y-5" key={group.title}>
              <div className="pb-1">
                <h3 className="font-serif text-3xl leading-none tracking-[-0.04em] text-ink sm:text-[2.2rem]">
                  {group.title}
                </h3>
                {group.description ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {group.items.map((item) => {
                  ...
                  return (
                    <article
                      className="overflow-hidden rounded-md border border-[#d9dfd7] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.98),rgba(244,246,240,0.96)_52%,rgba(229,236,226,0.92))] p-4 shadow-[0_24px_50px_rgba(17,24,20,0.06)]"
                      key={item.id}
                    >
                      <div className="relative mb-5 flex min-h-[15rem] items-end justify-center overflow-hidden rounded-sm border border-black/6 bg-[linear-gradient(180deg,rgba(255,255,255,0.85),rgba(240,244,236,0.7))] p-4">
                        ...
                        {item.imageSrc ? (
                          <Image
                            ...
                            className="relative z-10 object-contain drop-shadow-[0_18px_20px_rgba(17,24,20,0.18)]"
                            fill
                      ...
                      <div className="space-y-2">
                        ...
                        {displayVolume ? (
                          <p className="text-5xl font-semibold uppercase leading-none tracking-[-0.08em] text-ink sm:text-6xl">
                            {displayVolume}
                          </p>
                        ) : null}
                        <h4 className="font-serif text-[1.9rem] leading-none tracking-[-0.04em] text-ink">
                          {item.name}
                        </h4>
                        {displayQualifier ? (
